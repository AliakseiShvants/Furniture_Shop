 ...
 */
CREATE TABLE furniture_shop.ROLES(
 ...
 */
CREATE TABLE furniture_shop.REQUISITES
(
...
(
  ...
  FOREIGN KEY (ROLE_ID) REFERENCES furniture_shop.ROLES(ROLE_ID)
);
 ...
 */
CREATE TABLE furniture_shop.MANUFACTURERS
(
  MANUFACTURER_ID BIGSERIAL PRIMARY KEY ,
  REQUISITE_ID BIGINT,
  TITLE VARCHAR(100) NOT NULL ,

  FOREIGN KEY (REQUISITE_ID) REFERENCES furniture_shop.REQUISITES(REQUISITE_ID)
);

/**
  A table that represents categories of products
 */
CREATE TABLE furniture_shop.CATEGORIES
(
 ...
 */
CREATE TABLE furniture_shop.PRODUCTS
(
  ...
  FOREIGN KEY (MANUFACTURER_ID) REFERENCES furniture_shop.MANUFACTURERS (MANUFACTURER_ID),
  FOREIGN KEY (CATEGORY_ID) REFERENCES furniture_shop.CATEGORIES (CATEGORY_ID)
);

/**
  A table with images of products
 */
CREATE TABLE furniture_shop.PRODUCT_IMAGES
(
  PRODUCT_IMAGE_ID BIGSERIAL PRIMARY KEY ,
  PRODUCT_ID BIGINT NOT NULL ,
  URL VARCHAR(500) NOT NULL ,

  FOREIGN KEY (PRODUCT_ID) REFERENCES furniture_shop.PRODUCTS(PRODUCT_ID)
);

/**
  A table that stores sailing info about product (code, price, quantity)
 */
CREATE TABLE furniture_shop.STORAGE_ITEMS
(
...
/**
  A table that represents order status
 */
CREATE TABLE furniture_shop.ORDER_STATUSES
(
...
(
  ...
  FOREIGN KEY (MANAGER_ID) REFERENCES furniture_shop.USERS(USER_ID),
  FOREIGN KEY (STATUS_ID) REFERENCES furniture_shop.ORDER_STATUSES(ORDER_STATUS_ID)
);

/**
  A table with order items (products and their quantity)
 */
CREATE TABLE furniture_shop.ORDER_ITEMS
(
 ...
 */
CREATE TABLE furniture_shop.BASKET_ITEMS
(
